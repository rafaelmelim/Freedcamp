...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE indexname = 'labels_name_owner_id_key'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE policyname = 'Users can create their own labels'
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can view their own labels'
    AND tablename = 'labels'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can update their own labels'
    AND tablename = 'labels'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can delete their own labels'
    AND tablename = 'labels'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Users can manage labels for their tasks'
